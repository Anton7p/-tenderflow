import { FooterFieldsDto } from '../upload-excel.types';
import { normalize } from './upload-excel-parser.utils';

export function extractFooterFields(rows: (string | number | null)[][]): FooterFieldsDto {
  const textRows = rows.map((row) => row.map((cell) => toCellRaw(cell)));
  const totals = findItemTotals(textRows);

  return {
    pagesInfo:
      findValueOrInlineAfterLabel(textRows, 'Документ составлен на') ||
      findValueOrInlineAfterLabel(textRows, 'Документ на'),
    directorPositionName:
      findValue(textRows, 'Руководитель организации') ||
      findValue(textRows, 'или иное уполномоченное лицо', 0),
    chiefAccountantPositionName:
      findValue(textRows, 'Главный бухгалтер') ||
      findValue(textRows, 'или иное уполномоченное лицо', 1),
    transferBasis:
      findValueBeforeMarker(textRows, '[8]') || findValue(textRows, 'Основание передачи'),
    transportData:
      findValueBeforeMarker(textRows, '[9]') || findValue(textRows, 'Данные о транспортировке и грузе'),
    ...
    sellerResponsiblePosition:
      findFirstBeforeMarker(textRows, '[13]') ||
      findValue(textRows, 'Ответственный за правильность оформления факта хозяйственной жизни ', 0),
    sellerResponsible:
      findLastBeforeMarker(textRows, '[13]') ||
      findValue(textRows, 'Ответственный за правильность оформления факта хозяйственной жизни ', 0),
    buyerResponsiblePosition:
      findPenultimateBeforeMarker(textRows, '[18]') ||
      findValue(textRows, 'Ответственный за правильность оформления факта хозяйственной жизни', 0),
    buyerResponsible:
      findLastBeforeMarker(textRows, '[18]') ||
      findValue(textRows, 'Ответственный за правильность оформления факта хозяйственной жизни', 0),
    sellerEntityName:
      findValueBeforeMarker(textRows, '[14]') ||
      findValueExact(
        textRows,
        'Наименование экономического субъекта – составителя документа (в т.ч. комиссионера / агента)',
      ),
    buyerEntityName:
      findValueBeforeMarker(textRows, '[19]') ||
...
function findValueOrInlineAfterLabel(rows: string[][], label: string, occurrence = 0): string {
...
function findValueExact(rows: string[][], label: string, occurrence = 0): string {
  const normalizedLabel = normalizeLabelText(label);
  let seen = 0;
  ...
  for (const row of rows) {
    ...
    const candidate = row
      ...
      .find((cell) => {
        ...
        if (
          value.includes('данные о транспортировке') ||
...
function toCellRaw(value: string | number | null): string {
...
function findFirstByContains(rows: string[][], needle: string): string {
...
function findFirstBeforeMarker(rows: string[][], marker: string): string {
...
function findLastBeforeMarker(rows: string[][], marker: string): string {
  const normalizedMarker = normalizeLabelText(marker);
  for (const row of rows) {
    const markerIndex = row.findIndex((cell) => normalizeLabelText(cell) === normalizedMarker);
    if (markerIndex < 0) {
...
function findPenultimateBeforeMarker(rows: string[][], marker: string): string {
  const normalizedMarker = normalizeLabelText(marker);
  for (const row of rows) {
    const markerIndex = row.findIndex((cell) => normalizeLabelText(cell) === normalizedMarker);
    if (markerIndex < 0) {
...
function findItemTotals(rows: string[][]): {
  ...
  for (const row of rows) {
    ...
    const anchorIndex = normalizedRow.findIndex((cell) =>
      anchorPhrases.some((phrase) => cell.includes(phrase)),
    );
    if (anchorIndex < 0) {
      continue;
    }
...
function normalizeNumericCell(value: string): string {
  ...
  // Форматы вроде 76,055.89 / 76.055,89 -> оставляем только десятичный разделитель.
  if (hasComma && hasDot) {
