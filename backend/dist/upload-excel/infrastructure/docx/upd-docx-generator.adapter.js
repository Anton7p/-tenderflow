...
const docx_template_fields_1 = require("./docx-template-fields");
let UpdDocxGeneratorAdapter = class UpdDocxGeneratorAdapter {
    ...
    buildTemplateData(payload) {
        ...
        const uppercaseAliases = {
            ...
            SHIPMENT_ADDITIONAL_INFO: templateFields['shipment_additional_info'] ?? templateFields['additional_info'] ?? '',
            ...
            BUYER_RECEIVER_POSITION: templateFields['buyer_receiver_position'] ?? templateFields['receiver_position'] ?? '',
    ...
    buildUpdDocumentTitle(payload, prebuiltTemplateFields) {
        const templateFields = prebuiltTemplateFields ?? {
            ...(0, docx_template_fields_1.buildDocxTemplateFields)(payload),
            ...(payload.docxFields ?? {}),
        };
        const documentNumber = templateFields['invoice_number']?.trim() ||
            templateFields['document_number']?.trim() ||
            payload.headerFields.documentNumber ||
            '';
        const documentDateHuman = this.toRussianHumanDate(templateFields['invoice_date'] ||
    ...
    formatRussianDateParts(day, month, year) {
        const months = [
            'января',
            'февраля',
            'марта',
